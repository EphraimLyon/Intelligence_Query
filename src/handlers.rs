...
pub async fn get_profiles(
    ...
    if let Some(name) = &filters.name {
        conditions.push(format!("name LIKE ${}", bindings.len() + 1));
        bindings.push(format!("%{}%", name));
    }

    if let Some(country) = &filters.country {
        conditions.push(format!("country_name = ${}", bindings.len() + 1));
        bindings.push(country.clone());
    }

    if let Some(country_id) = &filters.country_id {
        conditions.push(format!("country_id = ${}", bindings.len() + 1));
    ...
    if let Some(gender) = &filters.gender {
        conditions.push(format!("gender = ${}", bindings.len() + 1));
...
pub async fn search_profiles(
    ...
    if let Some(age_group) = &params.age_group {
        conditions.push(format!("age_group = ${}", bindings.len() + 1));
